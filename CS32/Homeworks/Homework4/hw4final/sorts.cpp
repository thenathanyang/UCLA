...
void insertion_sort(vector<Sensor>& s, bool comp(const Sensor&, const Sensor&))
{
    ...
    //     for (int k = i; k > 0 && comp(s[k], s[k-1]); k--)
    //         swap(s[k], s[k-1]);
...
int main()
{
    ...
    vector<Sensor*> vecSensors;
    for (int k = 0; k < auxSensors.size(); k++)
        vecSensors.push_back(&auxSensors[k]);


      // DONE:  Sort the vector of pointers using the STL sort algorithm
      //        with compareSensorPtr as the ordering relationship.

    sort(vecSensors.begin(), vecSensors.end(), compareSensorPtr);

    // for (int k = 0; k < vecSensors.size(); k++)
    //     cout << vecSensors[k]->avg << endl;
